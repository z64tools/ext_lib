...
#define EXTRACT_NUMBER_AND_INCR(destination, source) \
    ...
    #ifdef DEBUG
static void PREFIX(extract_number_and_incr) (
    int* destination,
    UCHAR_T** source
);
static void
...
PREFIX(print_double_string) (
    const CHAR_T * where, const CHAR_T*string1,
...
convert_mbs_to_wcs (CHAR_T* dest, const unsigned char* src, size_t len,
    ...
    offset_buffer[0] = 0;
    for (
        ; mb_remain > 0 ; ++wc_count, ++pdest, mb_remain -= consumed,
        psrc += consumed
    ) {
        #ifdef _LIBC
...
#define PUSH_FAILURE_POINT(pattern_place, string_place, failure_code) \
    do {                                                              \
        ...
        DEBUG_PRINT_DOUBLE_STRING (                                            \
            string_place, string1, size1, string2,                             \
            size2                                                              \
        );                                                                     \
        DEBUG_PRINT1 ("'\n");                                                  \
...
static void PREFIX(store_op2) (
    re_opcode_t op, UCHAR_T* loc,
    int arg1, int arg2
);
static void PREFIX(insert_op1) (
    re_opcode_t op, UCHAR_T* loc,
    int arg, UCHAR_T* end
);
static void PREFIX(insert_op2) (
    re_opcode_t op, UCHAR_T* loc,
...
handle_plus:
            case '*':
                ...
                
                {
                    ...
                    GET_BUFFER_SPACE (1 + OFFSET_ADDRESS_SIZE);
                    INSERT_JUMP (
                        keep_string_p ? on_failure_keep_string_jump
                                       : on_failure_jump,
                    ...
                    if (!zero_times_ok) {
                        ...
                        GET_BUFFER_SPACE (1 + OFFSET_ADDRESS_SIZE);
                        INSERT_JUMP (
                            dummy_failure_jump, laststart, laststart +
            ...
            case '[': {
                ...
                for (;;) {
                    ...
                    if (
                        ...
                        b += 2;
                        ret = wcs_compile_range (
                            range_start, &p, pend, translate,
                            syntax, b, laststart
                        );
...
normal_char:
       ...
       the search and match routines.  */
    {
        ...
        if (fail_stack.size < (2 * re_max_failures * MAX_FAILURE_ITEMS)) {
            ...
            if (!fail_stack.stack)
                fail_stack.stack
                    = (PREFIX(fail_stack_elt_t)*)xmalloc (
                        fail_stack.size
                        * sizeof (PREFIX(fail_stack_elt_t))
                    );
            else
                fail_stack.stack
                    = (PREFIX(fail_stack_elt_t)*)xrealloc (
                        fail_stack.stack,
                        (fail_stack.size
                        * sizeof (PREFIX(fail_stack_elt_t)))
                    );
            ...
            if (!fail_stack.stack)
                fail_stack.stack
                    = (PREFIX(fail_stack_elt_t)*)malloc (
                        fail_stack.size
                        * sizeof (PREFIX(fail_stack_elt_t))
                    );
            else
                fail_stack.stack
                    = (PREFIX(fail_stack_elt_t)*)realloc (
                        fail_stack.stack,
                        (fail_stack.size
                        * sizeof (PREFIX(fail_stack_elt_t)))
                    );
...
static void
PREFIX(insert_op2) (
    re_opcode_t op, UCHAR_T*loc, int arg1,
...
group_in_compile_stack (compile_stack_type compile_stack, regnum_t regnum) {
    ...
    for (
        this_element = compile_stack.avail - 1;
        this_element >= 0;
        this_element--
    )
...
wcs_compile_range (CHAR_T range_start_char, const CHAR_T** p_ptr,
    ...
    if (nrules != 0) {
        const char* collseq = (const char*) _NL_CURRENT(
            LC_COLLATE,
...
byte_compile_range (unsigned int range_start_char, const char** p_ptr,
    ...
    ret = syntax & RE_NO_EMPTY_RANGES ? REG_ERANGE : REG_NOERROR;
        ...
        #if _LIBC
    collseq = (const unsigned char*) _NL_CURRENT (
        LC_COLLATE,
...
re_search (struct re_pattern_buffer* bufp, const char* string, int size,
    ...
    return re_search_2 (
        bufp, NULL, 0, string, size, startpos, range,
        regs, size
    );
}
    #ifdef _LIBC
...
re_search_2 (struct re_pattern_buffer* bufp, const char* string1, int size1,
    ...
    #ifdef MBS_SUPPORT
    if (MB_CUR_MAX != 1)
        return wcs_re_search_2 (
            bufp, string1, size1, string2, size2, startpos,
            range, regs, stop
        );
    else
    #endif
...
static int
PREFIX(re_search_2) (
    struct re_pattern_buffer* bufp, const char* string1,
    ...
    if (size1 != 0) {
        ...
        wcs_size1 = convert_mbs_to_wcs(
            wcs_string1, string1, size1,
    ...
    if (size2 != 0) {
        ...
        wcs_size2 = convert_mbs_to_wcs(
            wcs_string2, string2, size2,
    ...
    for (;;) {
        ...
        if (
            range >= 0 && startpos == total_size && fastmap
            && !bufp->can_be_null
        ) {
    #ifdef WCHAR
            FREE_WCS_BUFFERS ();
    ...
    #ifdef WCHAR
        val = wcs_re_match_2_internal (
            bufp, string1, size1, string2,
...
re_match (struct re_pattern_buffer* bufp, const char* string,
    ...
    int result;
        ...
        #ifdef MBS_SUPPORT
    if (MB_CUR_MAX != 1)
        result = wcs_re_match_2_internal (
            bufp, NULL, 0, string, size,
            pos, regs, size,
...
re_match_2 (struct re_pattern_buffer* bufp, const char* string1, int size1,
    ...
    if (MB_CUR_MAX != 1)
        result = wcs_re_match_2_internal (
            bufp, string1, size1, string2, size2,
            pos, regs, stop,
            NULL, 0, NULL, 0, NULL, NULL
        );
    else
    ...
    #endif /* BYTE */
{
    ...
    if (string1 == NULL && string2 == NULL) {
        ...
        if (csize2 != 0) {
            ...
            size2 = convert_mbs_to_wcs(
                string2, cstring2, csize2,
                mbs_offset2, is_binary
            );
...
succeed_label:
            ...
            if (regs && !bufp->no_sub) {
                ...
                for (
                    mcnt = 1; (unsigned) mcnt < MIN (num_regs, regs->num_regs);
                    mcnt++
                ) {
        ...
        switch (SWITCH_ENUM_CAST ((re_opcode_t) *p++)) {
            ...
            case exactn_bin:
                ...
                if (translate) {
                    do {
                        ...
                        if (*d <= 0xff) {
                            if (
                                (UCHAR_T) translate[(unsigned char) *d++]
                                != (UCHAR_T) *p++
                            )
                                goto fail;
                        } else {
            ...
            case charset_not: {
                ...
                if (nrules != 0) {
                    ...
                    for (
                        workp2 = workp + coll_symbol_length ; workp < workp2 ;
                        workp++
                    ) {
                        int32_t* wextra;
            ...
            #include <locale/weightwc.h>
                    ...
                    for (
                        workp2 = workp + equiv_class_length ; workp < workp2 ;
                        workp++
                    ) {
                        idx = (int32_t)*workp;
        ...
        #ifdef _LIBC
                        if (
                            __wcscoll (range_start_char, str_buf + 2) <= 0
                            && __wcscoll (str_buf + 2, range_end_char) <= 0
                        )
        #else
                        if (
                            wcscoll (range_start_char, str_buf + 2) <= 0
                ...
                DEBUG_PRINT2 ("EXECUTING maybe_pop_jump %d.\n", mcnt);
                {
                    ...
                    while (1) {
                        if (
                            ...
                            p2 + 2 + 2 * OFFSET_ADDRESS_SIZE < pend
                    ...
                    if (p2 == pend) {
                        ...
                        if (
                            ((re_opcode_t) p1[1 + OFFSET_ADDRESS_SIZE] == exactn
    #ifdef MBS_SUPPORT
                            ...
                            (re_opcode_t) p1[3] == charset
                            || (re_opcode_t) p1[3] == charset_not
                        ) {
                            int negate = (re_opcode_t) p1[3] == charset_not;
                            
                            if (
                                c < (unsigned) (p1[4] * BYTEWIDTH)
            ...
            if (p < pend) {
                ...
                switch ((re_opcode_t) *p) {
                    ...
                    case jump:
                        ...
                        if (
                            (is_a_jump_n && (re_opcode_t) *p1 == succeed_n)
                            || (!is_a_jump_n
...
PREFIX(bcmp_translate) (
    const CHAR_T * s1, const CHAR_T*s2, register int len,
