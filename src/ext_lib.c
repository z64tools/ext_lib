...
extern void osLogInit();
extern void osLogDestroy();

onlaunch_func_t extlib_init(void) {
	osLogInit();
...
onlaunch_func_t x_init() {
	gBufX.head = calloc(gBufX.max);
}

onexit_func_t x_dest() {
	delete(gBufX.head);
}
...
static char* __impl_snakeify(void* (*falloc)(size_t), const char* s) {
	char* new = falloc(strlen(s) * 2);
	
	return ifyize(new, s, tolower);
}

static char* __impl_externify(void* (*falloc)(size_t), const char* s) {
	char* new = falloc(strlen(s) * 2);
...
static char* __impl_strtrim(void* (*falloc)(size_t), const char* item, const char* reject) {
	int slen;
...
static char* __impl_stras8(void* (*falloc)(size_t), wchar* src) {
	char* dst = falloc((strwlen(src) + 1) * sizeof(char));
...
static wchar* __impl_stras16(void* (*falloc)(size_t), char* src) {
	wchar* dst = falloc((strlen(src) + 1) * sizeof(wchar));
...
char* x_snakeify(const char* s) {
	return __impl_snakeify(x_alloc, s);
}
char* x_externify(const char* s) {
	return __impl_externify(x_alloc, s);
}
...
char* x_strtrim(const char* item, const char* reject) {
	return __impl_strtrim(x_alloc, item, reject);
}
char* x_stras8(wchar* src) {
	return __impl_stras8(x_alloc, src);
}
wchar* x_stras16(char* src) {
	return __impl_stras16(x_alloc, src);
}
...
char* snakeify(const char* s) {
	return __impl_snakeify(m_alloc, s);
}
char* externify(const char* s) {
	return __impl_externify(m_alloc, s);
}
...
char* strtrim(const char* item, const char* reject) {
	return __impl_strtrim(m_alloc, item, reject);
}
char* stras8(wchar* src) {
	return __impl_stras8(m_alloc, src);
}
wchar* stras16(char* src) {
	return __impl_stras16(m_alloc, src);
}
...
void Color_Convert2hsl(hsl_t* dest, rgb8_t* src) {
	*dest = ColorHSL(UnfoldRGB(*src));
}

void Color_Convert2rgb(rgb8_t* dest, hsl_t* src) {
	rgba8_t c = ColorRGBA8(src->h, src->s, src->l);
...
bool memeq(const void* mema, const void* memb, size_t size) {
	const char* a = mema;
	const char* b = memb;
	const char* e = a + size;
...
void* memmem_align(u32 val, const void* haystack, size_t haystacklen, const void* needle, size_t needlelen) {
	...
	while (haystacklen - (p - bf) >= needlelen) {
		if (p[0] == s[0] && memeq(p, needle, needlelen))
			return p;
		p += val;
	}
...
char* strwstr(const char* hay, const char* nee) {
	...
	while (p) {
		if ((p == hay || !isgraph(p[-1])) && !isgraph(p[strlen(nee)]))
			return p;
...
typedef char* (*nsfunc)(const char*, const char*, size_t);

static int __impl_strnrep(char* str, size_t len, sfunc srch, nsfunc nsrch, const char* mtch, const char* rep, int num, int start) {
	char* o = str;
	...
	while (
		...
		(nsrch && (str = nsrch(str, mtch, max_len ? 0 : len - (str - o))) )
	) {
		if (start > 0) {
			start--;
			str++;
...
int strwrep(char* src, const char* mtch, const char* rep) {
	return __impl_strnrep(src, 0, strwstr, NULL, mtch, rep, -1, -1);
}

int strwnrep(char* src, int len, const char* mtch, const char* rep) {
	return __impl_strnrep(src, len - 1, strwstr, NULL, mtch, rep, -1, -1);
}
...
size_t strxspn(const char* str, int (*is)(int)) {
	const char* p = str;
...
void sys_setworkdir(const char* s) {
	if (dir_isrel(s))
		s = x_dirabs(s);
	
	if (chdir(s))
		errr("failed to set work dir: %s", s);
}
...
int sys_cp(const char* src, const char* dest) {
	if (sys_isdir(src)) {
		...
		for (int i = 0; i < list.num; i++) {
			...
			osLog("Copy: %s -> %s", list.item[i], dfile);
...
const char* sys_volumename(const char* volume) {
	char* vname = NULL;
		...
		wchar_t dest[64];
		static char sVolName['Z' - 'A' + 1][64];
		int vol = toupper(volume[0]) - 'A';
...
int sys_exe(const char* cmd) {
	...
	if (ret != 0)
		osLog(PRNT_REDD "[%d] " PRNT_GRAY "sys_exe(" PRNT_REDD "%s" PRNT_GRAY ");", ret, cmd);
...
int sys_exel(const char* cmd, int (*callback)(void*, const char*), void* arg) {
	...
	if ((file = popen(sys_exe_s(cmd), "r")) == NULL) {
		osLog(PRNT_REDD "sys_exes(%s);", cmd);
		osLog("popen failed!");
...
char* sys_exes(const char* cmd) {
	...
	osAssert(sExeBuffer != NULL);
	sExeBuffer[0] = '\0';
	
	while (fgets(result, 1024, file)) {
		size += strlen(result);
		osAssert (size < MbToBin(4));
	...
	if ((sSysReturn = pclose(file)) != 0) {
		if (sSysIgnore == 0) {
			printf("%s\n", sExeBuffer);
			osLog(PRNT_REDD "[%d] " PRNT_GRAY "sys_exes(" PRNT_REDD "%s" PRNT_GRAY ");", sSysReturn, cmd);
...
char* fs_find(const char* str) {
	...
	osLog("Find: [%s] in [%s]", str, sFileSys.path);
...
void cli_getPos(int* r) {
	...
	int y = 0;
		...
		osAssert (GetConsoleScreenBufferInfo(GetStdHandle(STD_OUTPUT_HANDLE), &cbsi));
		...
		tcsetattr(STDIN_FILENO, TCSAFLUSH, &a);
		printf("\033[6n");
		fflush(stdout);
		scanf("\033[%d;%dR", &y, &x);
		...
		tcsetattr(STDIN_FILENO, TCSAFLUSH, &b);
		fflush(stdout);
#endif // _WIN32
...
void freenullist(const char** list) {
	if (list)
		for (char** ptr = (char**)list; *ptr; ptr++)
			free(ptr);
}
...
void* crealloc(void* ptr, size_t old_size, size_t new_size) {
	...
	if (ptr != NULL && new_size - old_size > 0)
