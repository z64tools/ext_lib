...
NanoGrid_C       = $(shell cd $(PATH_EXTLIB) && find src/nanogrid/* -type f -name '*.c')
...
Icons            = $(shell cd $(PATH_EXTLIB) && find src/icons/* -type f -name '*.svg')
...
NanoGrid_Linux_O = $(foreach f,$(NanoGrid_C:.c=.o), bin/linux/$f)
...
All_Linux_O      = $(ExtLib_Linux_O) $(ExtGui_Linux_O) $(NanoGrid_Linux_O) \
					$(Zip_Linux_O) $(Audio_Linux_O) $(Mp3_Linux_O) $(Xm_Linux_O) \
...
NanoGrid_Win32_O = $(foreach f,$(NanoGrid_C:.c=.o), bin/win32/$f)
...
All_Win32_O      = $(ExtLib_Win32_O) $(ExtGui_Win32_O) $(NanoGrid_Win32_O) \
					$(Zip_Win32_O) $(Audio_Win32_O) $(Mp3_Win32_O) $(Xm_Win32_O) \
...
NanoGrid_Linux_O += $(foreach f,$(Icons:%=%.o), bin/linux/$f)
NanoGrid_Win32_O += $(foreach f,$(Icons:%=%.o), bin/win32/$f)
...
bin/win32/src/icons/%.o: $(PATH_EXTLIB)/src/icons/%
	@mkdir -p $(dir $@)
	@echo "$(PRNT_RSET)[$(PRNT_GREN)g$(ASSET_FILENAME)$(PRNT_RSET)]"
	@$(DataFileCompiler) --cc $(CC_W32) --i $< --o $@
...
bin/linux/src/icons/%.o: $(PATH_EXTLIB)/src/icons/%
	@mkdir -p $(dir $@)
	@echo "$(PRNT_RSET)[$(PRNT_GREN)g$(ASSET_FILENAME)$(PRNT_RSET)]"
	@$(DataFileCompiler) --cc gcc --i $< --o $@
